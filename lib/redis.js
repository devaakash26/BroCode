...
const redisAdapter = {
  ...
  async keys(pattern) {
    ...
    if (currentProvider === "upstash") {
      ...
      // Railway/ioredis supports keys command
      return await redisClient.keys(pattern);
    }
  },
...
const KEYS = {
  ...
  // Leaderboard
  LEADERBOARD: () => `brocode-leaderboard-default`,
};
...
export const redisHelpers = {
  ...
  cache, // Expose the cache object directly
  ...
  async invalidateLeaderboard() {
    await cache.del(KEYS.LEADERBOARD());
  },
  ...
  async getOnlineUserIds() {
    try {
      ...
      if (!isReady || !redisClient) {
        console.log(
          "[redisHelpers] getOnlineUserIds - Redis not available, using in-memory fallback",
        );
        return Array.from(inMemoryStore.onlineUsers.keys());
      }

      console.log(
        "[redisHelpers] getOnlineUserIds - Querying Redis for user:online:* keys",
      );
      const keys = await redisAdapter.keys("user:online:*");
      console.log(
        `[redisHelpers] getOnlineUserIds - Found ${keys.length} online users in Redis`,
      );

      return keys.map((key) => key.replace("user:online:", ""));
    } catch (error) {
      console.error("[redisHelpers] getOnlineUserIds error:", error);
      return [];
    }
  },
  ...
  async getOnlineUser(userId) {
    try {
      ...
      return await redisAdapter.get(`user:online:${userId}`);
    } catch (error) {
      console.error("Redis getOnlineUser error:", error);
      return null;
    }
  },
   ...
   */
  async setUserOnline(userId, userData, ttl = 1800) {
    try {
      if (!redisEnabled) return;
  ...
  async setUserOffline(userId) {
    try {
      ...
      await redisAdapter.del(`user:online:${userId}`);
    } catch (error) {
