...
import { notifyWhatsApp } from "@/app/lib/utils";
...
export const authOptions = {
  ...
  callbacks: {
    ...
    async signIn({ user, account, profile, isNewUser }) {
      ...
      const baseSignInResult = baseAuthOptions.callbacks.signIn
        ? await baseAuthOptions.callbacks.signIn({
            user,
            account,
            profile,
            isNewUser,
  ...
  events: {
    ...
    createUser: async ({ user }) => {
      ...
      // Notify via WhatsApp for every new user regardless of auth provider
      notifyWhatsApp(user.name || user.email.split("@")[0], user.email);
    },
  },
  pages: {
    signIn: "/auth/signin",
