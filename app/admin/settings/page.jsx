...
function Toast({ toast, onClose }) {
  ...
  return (
    <div
      className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 px-5 py-3 rounded-xl border backdrop-blur-md shadow-lg max-w-sm ${c[toast.type] || c.info}`}
    >
      {toast.type === "success" && (
...
export default function AdminSettingsPage() {
  ...
  const countdown = useCountdown(
    config?.emergency?.active ? config.emergency.endTime : null,
  );
  ...
  const handleActivateEmergency = () => {
    ...
    setConfirm({
      ...
      description:
        "All users will immediately be locked out. Admins keep full access.",
      ...
      onConfirm: async () => {
        ...
        const ok = await saveConfig({
          ...
          emergency: {
            ...
            endTime: fromLocalInput(emEndTime),
            reason: emReason,
            notifyOnComplete: emNotify,
          },
        });
  ...
  const handleDeactivateEmergency = () => {
    ...
    setConfirm({
      ...
      description: config?.emergency?.notifyOnComplete
        ? "Platform goes live. Subscribers receive email automatically."
      ...
      onConfirm: async () => {
        ...
        const ok = await saveConfig({
          ...config,
          emergency: { ...config.emergency, active: false },
        });
        if (ok)
          showToast("Maintenance deactivated — platform is live", "success");
      },
    });
  };
  ...
  const handleSchedule = async () => {
    ...
    const ok = await saveConfig({
      ...
      scheduled: {
        active: true,
        startTime: fromLocalInput(scStart),
        endTime: fromLocalInput(scEnd) || null,
        reason: scReason,
      },
    });
    if (ok)
      showToast(
        "Maintenance scheduled — users will see the alert banner",
        "success",
      );
  };
  ...
  const handleCancelScheduled = async () => {
    ...
    const ok = await saveConfig({
      ...config,
      scheduled: { active: false, startTime: null, endTime: null, reason: "" },
    });
    if (ok) {
      setScStart("");
      setScEnd("");
    ...
    <>
      ...
      {isTempAdmin && (
        <div className="mb-4 flex items-center gap-3 px-4 py-3 rounded-xl border border-amber-500/30 bg-amber-500/10">
          ...
          <p className="text-sm text-amber-700 dark:text-amber-300 font-medium">
            You have View Only access. All actions are disabled. Contact your
      ...
      <div className="space-y-6 pb-10">
        ...
        <div className="flex justify-end">
          <div
            className={`flex items-center gap-2.5 px-4 py-2 rounded-full border w-fit ${systemStatus.ring}`}
          >
            <span className="relative flex h-2.5 w-2.5">
              {(isEmergencyActive || isScheduledActive) && (
                <span
                  className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-60 ${systemStatus.dot}`}
                />
              )}
              <span
                className={`relative inline-flex rounded-full h-2.5 w-2.5 ${systemStatus.dot}`}
              />
            </span>
            <span
              className={`text-xs font-bold uppercase tracking-wider font-mono ${systemStatus.color}`}
            >
              {systemStatus.label}
            </span>
          </div>
        ...
        {/* Emergency */}
        <div
          className={`relative rounded-2xl border overflow-hidden transition-all duration-500 ${isEmergencyActive ? "border-red-500/40 bg-red-500/[0.04]" : "border-gray-200 bg-gray-50 dark:border-white/[0.08] dark:bg-white/[0.02]"}`}
        >
          {isEmergencyActive && (
            <div className="pointer-events-none absolute inset-0 rounded-2xl shadow-[0_0_50px_-10px_rgba(239,68,68,0.25)]" />
          )}
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-white/[0.06]">
            <div className="flex items-center gap-3">
              <div
                className={`w-9 h-9 rounded-xl flex items-center justify-center ${isEmergencyActive ? "bg-red-500/20 border border-red-500/30" : "bg-gray-100 border border-gray-200 dark:bg-white/[0.05] dark:border-white/10"}`}
              >
                <Zap
                  className={`h-4 w-4 ${isEmergencyActive ? "text-red-400" : "text-gray-400"}`}
                />
              </div>
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Instantly locks all users out of the platform
                </p>
              </div>
            </div>
            <div
              className={`px-3 py-1 rounded-full text-xs font-bold font-mono uppercase tracking-wider border ${isEmergencyActive ? "text-red-400 border-red-500/30 bg-red-500/10" : "text-gray-500 border-gray-200 bg-gray-50 dark:border-white/10 dark:bg-white/[0.03]"}`}
            >
            ...
            {isEmergencyActive ? (
              <div className="space-y-5">
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {[
                    {
                      label: "Started",
                      val: fmtDateShort(config.emergency.startTime),
                    },
                    {
                      label: "Est. End",
                      val: fmtDateShort(config.emergency.endTime),
                    },
                    {
                      label: "Waiting",
                      val: `${subscriberCount} subscriber${subscriberCount !== 1 ? "s" : ""}`,
                    },
                  ].map(({ label, val }) => (
                    <div
                      key={label}
                      className="bg-gray-50 border border-gray-100 dark:bg-white/[0.03] dark:border-white/[0.06] rounded-xl p-4"
                    >
                ...
                {countdown && (
                  <div>
                    ...
                    <div className="flex gap-3">
                      {[
                        { l: "hrs", v: String(countdown.h).padStart(2, "0") },
                        { l: "min", v: String(countdown.m).padStart(2, "0") },
                        { l: "sec", v: String(countdown.s).padStart(2, "0") },
                      ].map(({ l, v }) => (
                        <div
                          key={l}
                          className="flex flex-col items-center bg-red-500/10 border border-red-500/20 rounded-xl px-5 py-3 min-w-[72px]"
                        >
                          <span className="text-2xl font-bold font-mono tabular-nums text-red-600 dark:text-red-300">
                            {v}
                          </span>
                          <span className="text-[10px] text-red-500 dark:text-red-400/60 uppercase tracking-wider mt-0.5">
                            {l}
                          </span>
                        </div>
                      ))}
                ...
                <div className="flex flex-wrap gap-3 pt-1">
                  <button
                    ...
                    disabled={saving || subscriberCount === 0}
                  ...
                  >
                    ...
                    {saving
                      ? "Sending…"
                      : `Notify ${subscriberCount} subscriber${subscriberCount !== 1 ? "s" : ""}`}
                  </button>
                  <button
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="datetime-local"
                      value={emEndTime}
                      onChange={(e) => setEmEndTime(e.target.value)}
                      disabled={isTempAdmin}
                      className="w-full bg-gray-50 dark:bg-white/[0.04] border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-orange-500/50 transition-colors dark:[color-scheme:dark] disabled:opacity-50 disabled:cursor-not-allowed"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-mono text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">
                    ...
                    <input
                      ...
                      disabled={isTempAdmin}
                      className="w-full bg-gray-50 dark:bg-white/[0.04] border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-600 focus:outline-none focus:border-orange-500/50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    />
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <Toggle
                    checked={emNotify}
                    onChange={() => setEmNotify((v) => !v)}
                    disabled={isTempAdmin}
                  />
                  <div>
                    <p className="text-sm text-gray-700 dark:text-gray-300">
                      Auto-notify subscribers when maintenance ends
                    </p>
                    <p className="text-xs text-gray-500">
                      Emails sent automatically when you deactivate
                    </p>
                  </div>
                </div>
                <button
                ...
                <p className="text-xs text-gray-600">
                  ⚡ Takes effect immediately. Admins keep full access.
                </p>
              </div>
            )}
          </div>
        ...
        {/* Scheduled */}
        <div
          className={`relative rounded-2xl border overflow-hidden transition-all duration-500 ${isScheduledActive ? "border-amber-500/40 bg-amber-500/[0.03]" : "border-gray-200 bg-gray-50 dark:border-white/[0.08] dark:bg-white/[0.02]"}`}
        >
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-white/[0.06]">
            <div className="flex items-center gap-3">
              <div
                className={`w-9 h-9 rounded-xl flex items-center justify-center ${isScheduledActive ? "bg-amber-500/15 border border-amber-500/25" : "bg-gray-100 border border-gray-200 dark:bg-white/[0.05] dark:border-white/10"}`}
              >
                <Calendar
                  className={`h-4 w-4 ${isScheduledActive ? "text-amber-400" : "text-gray-400"}`}
                />
              </div>
              <div>
                <h2 className="text-sm font-semibold text-gray-800 dark:text-white">
                ...
                <p className="text-xs text-gray-500">
                  Plan ahead — users see an alert banner before it starts
                </p>
              </div>
            </div>
            <div
              className={`px-3 py-1 rounded-full text-xs font-bold font-mono uppercase tracking-wider border ${isScheduledActive ? "text-amber-400 border-amber-500/30 bg-amber-500/10" : "text-gray-500 border-gray-200 bg-gray-50 dark:border-white/10 dark:bg-white/[0.03]"}`}
            >
          ...
          <div className="p-6 space-y-5">
            {isScheduledActive && (
              <div className="bg-amber-500/[0.07] border border-amber-500/25 rounded-xl p-4 space-y-3">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    <p className="text-xs font-mono text-amber-500/60 uppercase tracking-wider mb-0.5">
                      Start
                    </p>
                    <p className="text-gray-900 dark:text-white font-medium">
                      {fmtDate(config.scheduled.startTime)}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs font-mono text-amber-500/60 uppercase tracking-wider mb-0.5">
                    ...
                    <p className="text-gray-900 dark:text-white font-medium">
                      {fmtDate(config.scheduled.endTime)}
                    </p>
                  </div>
                </div>
                {config.scheduled.reason && (
                  ...
                  <div className="flex items-center gap-3 px-4 py-2.5 bg-amber-500/10 border border-amber-500/25 rounded-lg text-sm text-amber-700 dark:text-amber-200">
                    ...
                    <span className="flex-1">
                      ...
                      <span className="font-medium">
                        {fmtDateShort(config.scheduled.startTime)}
                      </span>
                      {config.scheduled.endTime && (
                        <>
                          ...
                          until{" "}
                          <span className="font-medium">
                            {fmtDateShort(config.scheduled.endTime)}
                          </span>
                        </>
                      )}
                    </span>
                    <span className="text-xs text-amber-700/50 dark:text-amber-300/50">
                      ✕
                    </span>
                  </div>
                </div>
            ...
            <p className="text-xs text-gray-600">
              📣 Users see a dismissible alert strip — their dismiss state
              persists so it does not keep reappearing.
            </p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-gray-50 dark:border-white/[0.08] dark:bg-white/[0.02] overflow-hidden">
          <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-100 dark:border-white/[0.06]">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                Users who subscribed on the maintenance page
              </p>
            </div>
          </div>
