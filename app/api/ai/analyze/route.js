...
// Soft per-user rate limit (best-effort; a sliding-ish counter in Redis/memory).
// Protects free-tier quotas from a runaway client without needing atomic INCR.
...
export async function POST(request) {
  try {
    // Not configured -> 200 with available:false so the client degrades quietly
    // instead of surfacing a scary error on the solve page.
    if (!isAiConfigured()) {
    ...
    const analysis = await analyzeSubmission({ problem, code, language });
    return NextResponse.json(analysis, { status: 200 });
  } catch (error) {
    // Never let an AI failure look like a submission failure.
    console.error("AI analyze error:", error?.message || error);
    return NextResponse.json(
      { available: false, message: "AI analysis failed.", error: error?.message },
      { status: 200 },
    );
  }
}
