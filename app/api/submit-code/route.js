...
async function getLeaderboard(challengeId) {
  const participants = await prisma.challengeParticipant.findMany({
    where: { challengeId },
    orderBy: [{ score: "desc" }, { problemsSolved: "desc" }],
    include: {
      user: {
        select: {
          id: true,
...
export async function POST(request) {
  try {
    ...
    const testResults = finalResults.map((result, index) => ({
      ...
      expectedOutput:
        problem.testCasesRel[index].expectedOutput ||
        problem.testCasesRel[index].output,
      output: result.stdout
        ? result.stdout.trim()
        : (result.stderr || result.compile_output || "").trim(),
    ...
    const submission = await prisma.submission.create({
      data: {
        ...
        executionTime: Math.round(
          Math.max(...testResults.map((r) => r.executionTime || 0)) * 1000,
        ),
