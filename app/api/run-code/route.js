...
function wrapCodeWithBoilerplate(userCode, language, problem) {
  ...
  const hasLockedSections =
    userCode.includes("BEGIN LOCKED") && userCode.includes("END LOCKED");
    ...
    case "java":
      ...
      if (!userCode.includes("public static void main")) {
        ...
        if (!userCode.includes("class Solution")) {
          ...
          if (lastBraceIndex !== -1) {
            return (
              userCode.substring(0, lastBraceIndex) +
              "\n    // Auto-added for testing\n    public static void main(String[] args) {\n        Solution solution = new Solution();\n        // Example test will be run\n    }\n}"
            );
          }
        }
  ...
  try {
    if (!isJudge0Configured()) {
      return NextResponse.json(
        {
          message:
            "Code execution service is not configured. Please contact an administrator.",
        },
        { status: 500 },
      );
    }
    ...
    while (processing) {
      ...
      const getSubmissionsResponse = await axios.get(
        `${JUDGE0_URL}/submissions/batch?tokens=${submissionTokens.join(",")}&base64_encoded=false&fields=status_id,stdout,stderr,compile_output,token`,
        {
          headers: getJudge0Headers(),
        },
      );
      ...
      const stillProcessing = results.some(
