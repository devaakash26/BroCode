...
function DiffBadge({ d }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[11px] font-semibold uppercase tracking-wide ${c.bg} ${c.text}`}
    >
...
function Toggle({ on, onToggle, label, sublabel, icon: Icon }) {
  return (
    <div className="flex items-center justify-between py-3">
      ...
      >
        <span
          className={`absolute top-0.5 left-0.5 w-[18px] h-[18px] rounded-full bg-white shadow-sm transition-transform ${on ? "translate-x-[18px]" : ""}`}
        />
      </button>
    </div>
  );
...
export default function CreateChallengePage({ params }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const startTimeUtc = toUtcIso(startTime);
      const endTimeUtc = toUtcIso(endTime);
      ...
      const res = await fetch(`/api/groups/${groupId}/challenges`, {
        ...
        body: JSON.stringify({
          ...
          endTime: endTimeUtc,
          startTimezoneOffsetMinutes: startDate.getTimezoneOffset(),
          endTimezoneOffsetMinutes: endDate.getTimezoneOffset(),
          isPublic,
          strictMode,
          inviteOnly,
          lateEntryMinutes,
        ...
        <section className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 divide-y divide-zinc-100 dark:divide-zinc-800">
          ...
          <div className="p-4 sm:p-5">
            ...
            <textarea
              ...
              placeholder="Rules, goals, or notes for participants…"
              rows={3}
              maxLength={2000}
              ...
              <div className="mt-3 space-y-3">
                <div>
                  ...
                  <p className="text-[11px] text-zinc-400 dark:text-zinc-500 mb-1.5">
                    Participants can join up to this many minutes after start
                    time. After that, entry is blocked.
                  </p>
                  <input
                    type="number"
                    min={0}
        ...
        <section className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden">
          <div className="px-4 sm:px-5 py-3 border-b border-zinc-100 dark:border-zinc-800">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-medium text-zinc-500 dark:text-zinc-400 tabular-nums">
                {inviteOnly
                  ? `${invitedMembers.size} invited`
                  : `${members.length} members`}
              </span>
            </div>
            {!inviteOnly && (
              <p className="text-[11px] text-zinc-400 dark:text-zinc-500 mt-1">
                All group members can join this challenge. Turn on &quot;Invite
                only&quot; in step 1 to restrict.
              </p>
            )}
          </div>
                      ...
                      >
                        ...
                        {m.image ? (
                          <img
                            ...
                            onError={(e) => {
                              e.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(m.name)}&background=6366f1&color=fff&size=56`;
                            }}
                          />
                        ) : (
                          <img
                            src={`https://ui-avatars.com/api/?name=${encodeURIComponent(m.name)}&background=6366f1&color=fff&size=56`}
                            alt=""
                            className="w-7 h-7 rounded-full object-cover flex-shrink-0"
                          />
                        )}
            ...
            <div className="px-4 sm:px-5 py-3 border-t border-zinc-100 dark:border-zinc-800 bg-indigo-50/50 dark:bg-indigo-500/5">
              <div className="flex items-start gap-2">
                ...
                <p className="text-[11px] text-indigo-700 dark:text-indigo-300 leading-relaxed">
                  An email with the challenge link and schedule will be sent to
                  all {invitedMembers.size} invited member
                  {invitedMembers.size > 1 ? "s" : ""} when the challenge is
                  created.
                </p>
              </div>
            </div>
      ...
      {step === 3 && (
        <section className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 divide-y divide-zinc-100 dark:divide-zinc-800">
            ...
            <div className="space-y-1.5">
              {selectedProblems.map((p, i) => (
                <div key={p.id} className="flex items-center gap-2 text-sm">
                  <span className="text-zinc-400 dark:text-zinc-500 tabular-nums text-xs w-4 text-right">
                    {i + 1}.
                  </span>
                  <span className="text-zinc-800 dark:text-zinc-200 flex-1 truncate">
                    {p.title}
                  </span>
                  <DiffBadge d={p.difficulty} />
                </div>
          ...
          {inviteOnly && invitedMembers.size > 0 && (
            <div className="p-4 sm:p-5">
              <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-2">
                {invitedMembers.size} Invited Member
                {invitedMembers.size !== 1 ? "s" : ""}
              </p>
              <div className="flex flex-wrap gap-1.5">
                {invitedMembersList.map((m) => (
                  <span
                    key={m.id}
                    className="px-2 py-0.5 rounded-md bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 text-xs font-medium"
                  >
                    {m.name}
                  </span>
                ))}
