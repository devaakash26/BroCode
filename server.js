...
const memoryUsageMonitoring = setInterval(() => {
  if (process.env.NODE_ENV === "production") {
    ...
    if (memoryUsedMB > 900) {
      // Warning at 900MB
      console.warn(
        `High memory usage: ${memoryUsedMB}MB RSS, ${heapUsedMB}MB heap`,
      );
    }
    ...
    io.on("connection", (socket) => {
      ...
      socket.on("identify", (userData) => {
        if (userData?.id) {
          ...
          if (process.env.NODE_ENV === "development") {
            console.log(
              `User ${userData.id} (${userData.name || "Unknown"}) identified with socket ${socket.id}`,
            );
          }
        }
      });
      ...
      socket.on("sendMessage", async (data) => {
        try {
          ...
          for (const [id, connData] of socketConnections.entries()) {
            if (connData.socketId === socket.id) {
              userId = id;
              userData = connData.userData;
              break;
            }
          }
          ...
          const message = {
            ...
            sentAt:
              dbMessage?.sentAt?.toISOString() || new Date().toISOString(),
          };
